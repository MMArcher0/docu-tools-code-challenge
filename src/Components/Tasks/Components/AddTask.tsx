...
function AddTask() {
  const taskToEdit = useSelector(
    (state: any) => state.tasks.tasks[state.tasks.taskToEditId]
  );
  ...
  const handleAddTask = () => {
    ...
    dispatch(
      setTask({
        ...newTask,
        id:
          taskToEdit?.id && taskToEdit.status !== "completed"
            ? taskToEdit.id
            : uuidv4(),
      })
    );
  ...
  return (
    <>
      ...
      <TaskTextArea
        divStyle={{ width: "100%", marginBottom:"16px" }}
        title={"Description"}
      ...
      <div className="TaskFlexRow">
        <Button onClick={handleAddTask}>
          {taskToEdit && taskToEdit.status !== "completed"
            ? "Save changes"
            : "Add task"}
          {taskToEdit && taskToEdit.status !== "completed" ? (
            <FaSave color={"#FFFFFF"} size={23} />
          ) : (
            <FaPlus color={"#FFFFFF"} size={23} />
          )}
        </Button>
        {taskToEdit && taskToEdit.status !== "completed" && (
